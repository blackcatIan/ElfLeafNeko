...
public class ReceiveMsgList {
    ...
    private static ReceiveMsgList msgList= null;
    ...
    /**消息列队**/
    private static Vector<ReceiveMsgBean> msgListVector = new Vector<ReceiveMsgBean>();
    
    private ReceiveMsgList() {
    }
    
    //单例
    public synchronized static ReceiveMsgList getInstance() {
        if (msgList == null) {
            msgList = new ReceiveMsgList();
        }
     ...
     * 加入消息列队
     */
    public synchronized void add(ReceiveMsgBean msgBean) {
     ...
     * 提取一条消息进行处理
     */
    public synchronized ReceiveMsgBean peekMsgBean() {
        ReceiveMsgBean msgBean = msgListVector.size() > 0 ? msgListVector.get(0) : null;
     ...
     * @param index 列队中的序号
     */
    public synchronized void insetMsgBean(ReceiveMsgBean msgBean,int index) {
